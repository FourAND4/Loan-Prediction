...
# Tenor: satuan HARI, maks 480 hari, diurutkan TERLAMA -> TERCEPAT.
# Model tetap dilatih dalam satuan BULAN, jadi nilai hari dikonversi
# balik ke bulan (hari / 30) sebelum dikirim ke pipeline.
TENOR_OPTIONS = [480, 360, 300, 240, 180, 120, 84, 60, 36, 12]  # dalam hari, maks 480
...
with col_r:
    ...
    tenor_pinjaman_hari = st.selectbox(
        ...
        format_func=lambda x: f"{x} hari  (~{x / 30:.1f} bulan)",
    )
...
if predict_btn:
    ...
    fact_rows = [
        ...
        ("Tenor", f"{tenor_pinjaman_hari} hari (~{tenor_pinjaman:.1f} bulan)",
         "✅ Standar" if tenor_pinjaman_hari == 360 else "◇ Non-Standar"),
    ]
    ...
    if total_income > 0 and emi_bulanan > total_income * 0.5:
        recs.append((P["orange"], f"⚠️ Cicilan Terlalu Berat ({emi_bulanan / total_income * 100:.0f}% pendapatan)",
                     f"Pertimbangkan tenor lebih panjang (360–480 hari) untuk menurunkan cicilan bulanan."))
