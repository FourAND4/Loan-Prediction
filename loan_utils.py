...
source of truth). Ini WAJIB karena `pickle.dump()` menyimpan pipeline yang berisi
objek `RawCleaner` custom; saat `pickle.load()` dipanggil di app.py, Python butuh
...
def build_full_pipeline(clf, k_neighbors: int = 5) -> ImbPipeline:
    ...
    Karena semuanya jadi SATU objek, cukup simpan 1 file (.pkl) dan tidak
